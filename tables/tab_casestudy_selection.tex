% Notes: docs/exhibits.md#tab_casestudy_selection
\begin{table}[t]
  \centering
  \setlength{\abovecaptionskip}{2pt}%
  \setlength{\belowcaptionskip}{3pt}%
  \caption{Case-study setting and peer comparison.}
  \label{tab:casestudy_selection}

  {\scriptsize
  \renewcommand{\arraystretch}{1.15}%
  \setlength{\tabcolsep}{5pt}%

  \textbf{(a) Why this setting}\par\vspace{1pt}
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.40\columnwidth}
                  >{\raggedright\arraybackslash}p{0.47\columnwidth}}
    \toprule
    \textbf{Case-study choice} & \textbf{Rationale} \\
    \midrule
    12MP mode &
      Production-default capture resolution. \\
    \addlinespace
      No external memory pressure & Production-default condition without externally induced memory pressure. \\
    \addlinespace
      Run started at overheat Lv4 & Matches the motivating timeout condition and the production guard cutoff for optional stages. \\
    \bottomrule
  \end{tabular}

  \par\vspace{6pt}

  \textbf{(b) This run vs.\ peers}\par\vspace{1pt}
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.376\columnwidth}
                  >{\raggedleft\arraybackslash}p{0.075\columnwidth}
                  >{\raggedleft\arraybackslash}p{0.075\columnwidth}
                  >{\centering\arraybackslash}p{0.150\columnwidth}
                  >{\centering\arraybackslash}p{0.075\columnwidth}}
    \toprule
    \makecell[bl]{\textbf{Metric}} &
    \makecell[br]{\textbf{This}\\\textbf{run}} &
    \makecell[br]{\textbf{Peer}\\\textbf{med.}} &
    \makecell[bc]{\textbf{Peer}\\\textbf{[min--max]}} &
    \makecell[bc]{\textbf{vs.}\\\textbf{med.}} \\
    \midrule
    \(M\)~executed \(\uparrow\)
      & 8 & 14 & [8--30] & worse \\
    \(S\)~executed \(\uparrow\)
      & 22 & 30 & [22--30] & worse \\
    Pacing activated (\%)\,\(\downarrow\)
      & 55.2 & 31.0 & [13.8--69.0] & worse \\
    Cumulative applied delay (s)\,\(\downarrow\)
      & 5.21 & 4.17 & [3.51--9.70] & worse \\
    Slack \(P_5\) (\%)\,\(\uparrow\)
      & 5.2 & 9.4 & [4.6--15.7] & worse \\
    \bottomrule
  \end{tabular}}
\end{table}
