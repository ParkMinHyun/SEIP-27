...
%   Pearson r    +0.182 (12MP)  +0.136 (24MP)
%   Spearman     +0.242          +0.215
%   LS fit       d = 0.26 d* + 362 ms      d = 0.31 d* + 522 ms
%   d* quartiles  32 /  81 / 183 ms         74 / 197 / 310 ms
%   d  quartiles 324 / 393 / 472 ms        318 / 615 / 780 ms
% The applied delay occupies a narrow band while the requirement ranges over an
% order of magnitude, which is why the scatter reads as a horizontal cloud above
% the diagonal rather than as points along it.
%
...
% It does respond at the coarse level: for d* <= 100 ms the median applied delay
% is 370 ms in both conditions, and for d* > 300 ms it is 471 ms (12MP) and
% 699 ms (24MP).  Monotone, not proportional.
%
...
% resolve a pattern the two-band split hid.  Every column is now monotone in
% thermal level, and identically so in both conditions:
%   coverage    63.6 -> 96.0 -> 100.0 %   and   72.7 -> 83.3 -> 88.6 %
%   surplus P50  167 ->  340 ->   444 ms  and    188 ->  261 ->  301 ms
%   backlog P95 70.4 -> 63.9 ->  56.9 %   and   67.2 -> 66.2 ->  53.6 %
...
% both higher coverage and a lighter regulated backlog.  Under the two-band
% split the 12MP coverage read as 82.9 -> 100.0 with no visible middle, and the
% backlog trend was invisible.
% The low-band coverage figures rest on 11 required transitions each, so they
% are reported with their denominators and should not be quoted alone.
%
...
\begin{table}[t]
  ...
  the delay the controller's own condition requires on measured inputs. Across
  the 82 analyzed timeout-free sessions no transition fell below the
  mandatory-work floor, which was positive on 6 of 2,201 transitions and covered
  on all 6.}
  ...
  \begin{tabular}{|>{\centering\arraybackslash}m{25pt}%
    ...
    \multirow{4}{*}{\makecell[c]{12MP\\normal}}
      & \makecell[c]{Lv0--2\\{\scriptsize [11]}}
      & \makecell[r]{63.6\%\\{\scriptsize [7 of 11]}} & 94
      & 167 & $+302$ & $-67$ & 70.4\% \\
    \cline{2-8}
      & \makecell[c]{Lv3--4\\{\scriptsize [75]}}
      & \makecell[r]{96.0\%\\{\scriptsize [72 of 75]}} & 206
      & 340 & $+289$ & $-16$ & 63.9\% \\
    \cline{2-8}
      & \makecell[c]{Lv5--6\\{\scriptsize [8]}}
      & \makecell[r]{100.0\%\\{\scriptsize [8 of 8]}} & --
      & 444 & $+369$ & $-16$ & 56.9\% \\
    \cline{2-8}
      & \makecell[c]{\textit{All}\\{\scriptsize [94]}}
      & \makecell[r]{92.6\%\\{\scriptsize [87 of 94]}} & 206
      & 329 & $+296$ & $-19$ & 64.1\% \\
    \hline
    \multirow{4}{*}{\makecell[c]{24MP\\memory\\pressure}}
      & \makecell[c]{Lv0--2\\{\scriptsize [11]}}
      & \makecell[r]{72.7\%\\{\scriptsize [8 of 11]}} & 514
      & 188 & $+140$ & $+4$ & 67.2\% \\
    \cline{2-8}
      & \makecell[c]{Lv3--4\\{\scriptsize [18]}}
      & \makecell[r]{83.3\%\\{\scriptsize [15 of 18]}} & 322
      & 261 & $+249$ & $-44$ & 66.2\% \\
    \cline{2-8}
      & \makecell[c]{Lv5--6\\{\scriptsize [44]}}
      & \makecell[r]{88.6\%\\{\scriptsize [39 of 44]}} & 195
      & 301 & $+594$ & $-74$ & 53.6\% \\
    \cline{2-8}
      & \makecell[c]{\textit{All}\\{\scriptsize [73]}}
  ...
  \begin{minipage}{0.97\columnwidth}
    ...
    Because that reserve is the burst's observed maximum Draft duration, the
    delay holds an envelope over Draft cost rather than matching any one
    transition's need, and these columns ask whether the envelope covered the
    requirement rather than whether it equalled it.
    \(n_{\mathrm{req}}\) counts transitions with \(d^{*}>0\).
    \emph{Coverage} is the share of those transitions whose requirement was
    covered;
    \emph{Worst deficit} is the largest shortfall in ms among those not met.
    \emph{Surplus} is \(\max(0,d-d^{*})\) over paced transitions, summing to a
    per-burst median of \(3.11\) and \(2.69\)\,s out of a total applied delay
    that is \(18\%\) and \(14\%\) of the \(17.5\) and \(22.6\)\,s median burst.
    Where \(d\) and \(d^{*}\) are
    both positive, \(d-d^{*}=(\hat{C}-C)+(\hat{B}-B)/2\) holds identically, so
    the \emph{Reserve} and \emph{Backlog} columns split the surplus: the backlog
    clock is unbiased and the Draft reserve carries the cost. \emph{Backlog P95}
    is the 95th percentile of measured Draft backlog as a share of the timeout
    budget, i.e., the state the delay regulates rather than the delay itself.
    No Capture Timeout occurred, and the
    smallest realized margin --- \(0.3\)\% and \(0.2\)\% of the deadline ---
    belongs to no under-paced transition. Measured on a trace pacing
