...
% column, and no sentence in the note.  The RQ3 PROSE owns the minimum.  Both
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}%
    ...
    % ------------------------------------------------------------- 12MP normal
    % Condition only, as in (b).  The label used to read "79 of 1,920 decisions
    % required a delay (4.1%)" and carried three numbers that the table either
    % repeats or the prose owns: 79 is this block's own row sum (53+26+0), the
    % 4.1% share is a sentence of the RQ3 narrative rather than a cell, and the
    % analyzed total stays recoverable as (b)'s 1,841 plus that row sum.  The
    % paced count that once followed it was likewise the Paced column's sum.
    % Keep the labels at the condition: a block label is a single unwrappable
    % line spanning every column, so it is a floor on the table's width.
