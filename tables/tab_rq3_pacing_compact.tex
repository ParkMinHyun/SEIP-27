...
%
% d* itself is NOT defined in this table any more.  The RQ3 PROSE carries the
...
% burden moves to the RQ3 prose.  Never write that a worse coverage class was
...
\begin{table}[t]
  ...
  % defined in the RQ3 prose rather than here.  The table no longer carries the
  % formula anywhere, so that prose has to introduce it before this table is
  % read; see the d* note at the top of this file.
  \caption{RQ3: pacing coverage and delay sizing.}
  ...
  %   46  "$d^*_{man}\le d<d^*$" 46.0  (header "Coverage" is only 28.2)
  %   33  "11 (25.6\%)" 32.2            33  "83 (59.3\%)" 32.2
  %   12  "P50" 11.3                    14  "max" 12.8
  %   23  "P5 (\%)" 22.1                21  "$\hat C-C$" 19.6 / "$-29.6$" 20.6
  %   21  "$\hat B-B$" 19.9
  % Sum 203pt, so \fittabcolsep lands tabcolsep near 3.1pt.  A header line wider
  ...
  \begin{tabular}{@{}%
      >{\raggedright\arraybackslash}p{46pt}%  coverage condition
    ...
    \addlinespace[1pt]
    $d\ge d^{*}$              & 53 (67.1\%) & 53 (100\%)
      &   0 &   0 & 1.03 & $+41.4$ &  $-0.1$ \\
    $d^{*}_{\mathrm{man}}\!\le\!d\!<\!d^{*}$ & 26 (32.9\%) & 8 (30.8\%)
    ...
    \addlinespace[1pt]
    $d\ge d^{*}$              & 83 (59.3\%) & 83 (100\%)
      &   0 &   0 & 2.48 & $+44.4$ &  $-1.6$ \\
    $d^{*}_{\mathrm{man}}\!\le\!d\!<\!d^{*}$ & 43 (30.7\%) & 11 (25.6\%)
      & 131 & 530 & 1.20 &  $-0.9$ & $-19.8$ \\
    $d<d^{*}_{\mathrm{man}}$  & 14 (10.0\%) & 3 (21.4\%)
  ...
  \par\vspace{3pt}
  {\tiny\raggedright
  $d^{*}_{\mathrm{man}}$ is the part of $d^{*}$ that the mandatory Draft work
  alone requires.  $\hat{C}$ and $\hat{B}$ are the Draft reserve and the backlog clock
  held at the decision, so each error is estimate minus realized, as a share of
  the quantity it estimates and taken per decision before the median: class-wide
  in~(a), over the paced decisions alone in~(b).  $d/B$ is the delay as a share
  of the Draft work still outstanding while it ran, and \emph{inside} $B$ the
  share of the delay that elapsed against that work.  Pacing consumed 18.1 /
  24.5\% and 9.8 / 29.7\% of a run's elapsed time at P50 / P95.  Eleven of the
  3{,}781 decisions finished under 1\% slack and the tightest kept 0.11\%; on
  all eleven the backlog already held 42--79\% of the budget and pacing or an
  optional-work skip was engaged.  No analyzed run produced an observed Capture
  Timeout; realized slack is an outcome and not a deterministic bound, and
  pacing is closed-loop, so nothing here is a counterfactual.\par}
  }
\end{table}
