...
\begin{figure*}[t]
  ...
  \tikzset{
    ...
    % Applied on top of a policy style to draw its P10/P90 edge.
    rq3 band/.style={line width=0.3pt, opacity=0.55},
  }
  ...
  % P10/P90 edges are drawn first and thin so the medians stay legible on top.
  % Only the two collected policies carry edges; a pending CSV holds a header
  % only, so adding edges for it would draw nothing anyway.
  \newcommand{\rqthreepolicycurves}[2]{%
    \addplot[rq3 no pacing,rq3 band,forget plot]
      table[x=shot,y=#2_p10,col sep=comma]
      {#1/no_pacing.csv};
    \addplot[rq3 no pacing,rq3 band,forget plot]
      table[x=shot,y=#2_p90,col sep=comma]
      {#1/no_pacing.csv};
    \addplot[rq3 ours,rq3 band,forget plot]
      table[x=shot,y=#2_p10,col sep=comma]
      {#1/ours.csv};
    \addplot[rq3 ours,rq3 band,forget plot]
      table[x=shot,y=#2_p90,col sep=comma]
  ...
  backlog, queue depth, pacing delay, and the backlog--delay comparison.  Thick
  lines are per-shot medians and thin lines of the same style are the P10 and
  P90 edges over the nine No-pacing and eight Ours runs.
