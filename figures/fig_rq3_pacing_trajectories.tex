% RQ3: Per-shot policy trajectories and backlog-cost frontier for two devices.
%
% The scaffold intentionally renders "Data pending" until each device's policy
% traces have been collected.  After filling the CSV files under the matching
% data/rq3/<device>/ directory, set that device's data switch to true.
%
...
\begin{figure*}[t]
  ...
  \providecommand{\rqThreeUltraDataSwitch}{\rqthreeultrahasdatafalse}
  ...
  median backlog, queue depth, pacing delay, and the backlog--delay frontier.
  Panels (d) and (h) sweep each policy's calibrated parameter.}
