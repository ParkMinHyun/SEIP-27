...
%     together read as one thick rule at this scale.  The outer two bands are also
%     the two boundary classes of (b): the loosest band with d > 0 is
%     safe-but-paced, the overrun band with d = 0 is overrun-but-unpaced.
%
...
%
%     Both axes are in seconds, and NOT in points of budget.  The applied delay is
%     also printed in milliseconds in the table, and a budget-relative delay
%     beside a millisecond one recovers the budget by division in a single step;
%     the same is not true of two absolute forms.  This is the one place in the
%     RQ3 exhibits where the budget-relative unit is the disclosing one.
%
%     The frame stops at d = 2.5 s while B runs to 6.3 s, so the triangle is
%     bounded on the right by the frame rather than by the data.  Nothing hides
%     above it: the largest applied delay in either condition is 2.20 s, so every
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{groupplot}[
      ...
      \nextgroupplot[
        ...
        xlabel={Backlog $B$ (s)},
        ylabel={Applied delay $d$ (s)},
        xmin=0,xmax=6.3,ymin=0,ymax=2.5,
        xtick={0,2,4,6},
        ytick={0,1,2},
        yticklabel style={/pgf/number format/fixed},
      ]
      % The region a wait would have to reach to outlast the work it drains.  It
      % is bounded on the right by the frame, not by the data: the largest
      % applied delay is 2.20 s, so no transition with d > B falls outside it.
      \path[rq3pol region]
        (axis cs:0,0) -- (axis cs:2.5,2.5) -- (axis cs:0,2.5) -- cycle;
      \addplot[rq3pol zero,forget plot] coordinates {(0,0) (2.5,2.5)};
      \addplot[rq3pol scatter a,forget plot]
        table[x=backlog_s,y=delay_s,col sep=comma]
      ...
      % stay legible and then reads as a hole in the panel.  No mark reaches this
      % strip -- the largest applied delay is 2.20 s -- so the label needs no fill.
      % Three short lines rather than one: the region narrows toward its apex, and
      % a single line long enough to hold the count runs out through the rule.
      \node[rq3pol note,anchor=north west,align=left] at (axis cs:0.12,2.47)
        {$d>B$\\0 and 8\\waits};
      ...
      \nextgroupplot[
        ...
        xmin=-1.5,xmax=56,ymin=-0.55,ymax=1.72,
      ...
      \addlegendentry{P95}
      \node[rq3pol note,anchor=south west] at (axis cs:1.2,1.50) {never paced};
    \end{groupplot}
  \end{tikzpicture}
  \caption{RQ3 pacing policy evidence. (a) Activation against the retrospective
  pressure the controller never observed; the band is the pooled rate and the
  whiskers are a burst-cluster bootstrap. (b) The two boundary mismatches against
  ...
  waits reach it. (d) One mark per 30-shot burst; the column at zero is the 19 and
  13 bursts that never paced.}
