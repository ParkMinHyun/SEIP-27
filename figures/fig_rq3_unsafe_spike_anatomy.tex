  ...
  \begin{tikzpicture}
    ...
    \addplot[draw=black,fill=white,line width=0.35pt,bar shift=-6.5pt]
      coordinates {(-0.40,5) (-0.41,4) (0.08,3)
                   (-0.24,2) (-0.04,1)};
    \addlegendentry{Busy-core proxy}
    ...
    \node[font=\tiny,inner sep=0pt,anchor=south west]
      at (axis cs:1.66,5.62) {\textsf{replay safeguard}};
    \rqtwosg{5}{watchdog}
    \rqtwosg{4}{watchdog}
    \rqtwosgtwo{3}{later \(S\)}{skip}
    \rqtwosgtwo{2}{earlier \(M\)}{skip}
    \rqtwosgtwo{1}{earlier \(M\)}{skip}

    \end{axis}
  \end{tikzpicture}
  \caption{Unsafe admission decisions and replay safeguards.}
