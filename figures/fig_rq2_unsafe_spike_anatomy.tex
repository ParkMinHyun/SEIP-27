...
\begin{figure}[t]
  ...
  Latency and CPU time rose in all five cases, but the CPU-time bar is the
  shorter of the two everywhere except M3; the difference is taken up by busy
  cores, which fell in four cases and rose only where M3's latency barely
  moved.  Latency is CPU time divided by busy cores, a multiplicative
  ...
  shipped build would have applied.  \textsf{watchdog}: the per-node
  watchdog would have cut the decision node at its own budget.
  \textsf{later single-frame skip}: omitting the later Decoration work reduces
  the remaining cost; \textsf{earlier multi-frame skip}: omitting the earlier
  Bokeh work frees budget by shortening the path to the decision.  Every row
  carries a reason, so
  none of the five reaches a shipped build as a Capture Timeout; M1--M2 are the
  two the model's skips alone do not cover, and so the two that stay over
