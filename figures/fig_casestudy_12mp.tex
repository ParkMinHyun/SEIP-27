...
% this setting first times out: captures 8--12, the controller-off Timeout
% onset E/M of the 12MP normal Lv4 row of RQ1(a)
...
% with it, so cite RQ1(a) and not them when the prose names the window: Table I
% (\ref{tab:timeout_index}, normal capture, 12MP, M+S, starting level 4) reads
% 10/17 from the Section 2 motivation campaign, and the RQ1(b) No-control row
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{groupplot}[
      ...
      % The 1500 ms axis of Figure~\ref{fig:rq3_pacing_calibration} is a
      % different population -- delays there reach 2022 ms across all runs --
      % and the two panels are not meant to share a scale.
      \nextgroupplot[
