\subsection{Limitations of the Thermal Guard}
\label{sec:guard-limit}
The goal is to enable lightweight multi-frame composition under parallel capture whenever the draft sequence can finish within its deadline.

\smallskip\noindent\textbf{Production thermal guard.}
Samsung's overheat level is an ordinal thermal indicator on a 0--6 scale, where larger values denote stronger throttling.
During pre-release validation, throttling at level 4 delayed a single-frame draft stage and caused a Capture Timeout.
The failure recurred despite optimization, so we introduced a thermal guard that skips optional stages at level 4 or higher.
Because the draft sequence is implemented in framework code shared across device models, the same cutoff applies to all supported models.

The overheat level does not indicate a capture's waiting time or remaining draft work.
The guard may therefore skip optional stages even when enough time remains to complete the draft sequence.
Ambient heat, charging, or preceding device workloads can also raise the overheat level to 4 during ordinary use.
If a session starts at this level, the guard skips optional stages even for its first capture.

\smallskip\noindent\textbf{Motivating experiment.}
We compared two Portrait configurations on the Galaxy S26 Ultra with the thermal guard bypassed.
The optional stages are the lightweight multi-frame draft stage (\(M\)) and the deployed single-frame stages (collectively \(S\)).
Both \(M\) alone and \(M+S\) retain the same mandatory encoding and saving stages.
We tested 12MP and 24MP capture under normal memory conditions and under memory pressure.
For each condition in Table~\ref{tab:timeout_index}, we ran ten trials of up to 30 consecutive captures.
We chose 30 captures to extend testing beyond the earlier level-4 validation failure near the 20th consecutive capture.
The table reports the earliest timeout capture index across the ten trials, grouped by starting overheat level; a dash denotes no timeout within 30 captures.

\input{tables/tab_timeout_index.tex}

With \(M+S\), timeouts occurred across the full range of starting overheat levels, including runs that started below the guard's cutoff.
Even on the same device, the earliest timeout varied with resolution, memory pressure, and draft sequence configuration.
Using \(M\) alone delayed the earliest timeout or resulted in no observed timeout within 30 captures, so which optional stages execute also affects deadline safety.

\smallskip\noindent\textbf{Need for runtime control.}
The thermal cutoff does not capture how earlier draft sequences consume a later capture's timeout budget.
Skipping optional stages shortens that capture's processing but cannot recover time already spent waiting.
Executing them can also lengthen the wait for later captures.
Delaying the next capture can limit queue growth, but lengthens the shot-to-shot interval.
These constraints motivate coordinated runtime control of optional stages and capture arrivals.
