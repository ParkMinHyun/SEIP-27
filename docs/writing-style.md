...
| 2.3, Draft Sequence | Why does the camera need this sequence, and why is extending it difficult? | Define the Draft result and its relationship to the final image -> explain replacement and recovery -> explain the single worker and the costs of concurrent execution -> describe the sequence's evolution and deferred post-processing -> motivate lightweight multi-frame composition and its deadline constraint. |
| 2.4, Limitations of Static Thermal Gating | Why is the existing safeguard insufficient for deciding when the feature can run? | Define the thermal indicator and explain the failure that led to the guard -> explain what the cutoff does not observe -> introduce the capture trials and their conditions -> interpret the reported outcomes -> derive the need to control optional-stage execution and future capture arrivals together. |
