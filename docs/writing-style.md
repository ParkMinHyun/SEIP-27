...
point \(j\). Pacing keeps \(i\) as the capture index: capture \(i\)'s
request triggers the decision at \(t_i\), \(T_i\) is the window that
decision reads for the capture whose Draft Sequence is predicted to complete
at \(t^{\mathrm{end}}\), and \(d_i\) applies to the next capture
opportunity, which is what lets Section 3.1 pair pacing with
\(\delta_{i+1}\). The shared letter identifies the common deadline-window
concept; the decision point identifies the reading. Do not restore
admission's former \(R_{i,j}\) notation.
