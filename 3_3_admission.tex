...
A capture's timeout budget decreases while it waits for the draft worker and executes earlier stages.
\emph{Remaining-Sequence Admission} therefore runs immediately before each optional stage and compares the estimated duration of the entire remaining sequence, including mandatory encoding and saving, with the live budget.
...
At draft sequence completion, it retains the keys with positive measured durations \(p(k)\) from each recorded remaining sequence to form the measured sequence \(\mathcal K\).
