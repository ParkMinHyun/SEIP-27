...
- Sections 3.1 and 3.2 were merged into `3_1_overview.tex` on 2026-08-15. `3_2_objective.tex` is deleted, and the four files after it were renumbered down by one to keep file names aligned with subsection numbers: workload model `3_2_workload_model.tex`, admission `3_3_admission.tex`, pacing `3_4_pacing.tex`, implementation `3_5_implementation.tex`. Labels were not renamed, so `sec:model`, `sec:admission`, `sec:pacing`, and `sec:implementation` still resolve; cite subsections by label, never by hard-coded number. They were split before, with a rule policing which subsection owned which fact, and both opened on the same premise. The merged subsection is ordered by three run-in headings — `Controller architecture.` for the controller and its two modules, `Safety and responsiveness.` for the constraint and the two costs, `Cross-shot margin dynamics.` for the recursion, the two levers, and the asymmetry. Keep each fact under exactly one heading, and keep the asymmetry next to the levers the equation exposes rather than in the opening. `\label{sec:objective}` belongs on the `\subsection` line: `_4_experiments.tex` references it, and a `\label` placed after a run-in heading silently anchors to the preceding float instead.
