...
| RQ4 | Pacing-delay sizing | RQ3 | `tables/tab_rq4_pacing_sizing.tex` | RQ3, `docs/rq-evidence.md`, `data/rq3/`, `scripts/rq3_*.py` |
...
- The current main-paper exhibit is `tables/tab_rq4_pacing_sizing.tex` alone, and RQ3 ships no figure. It succeeded `tables/tab_rq4_pacing_selectivity.tex` on 2026-08-21, which had itself replaced `tables/tab_rq4_pacing_summary.tex` on 2026-08-13: the summary table measured $d$ against the retrospective target $d^{*}$, which is algebraically an estimator report at a fixed coefficient, whereas both later tables ask whether the delay is selective, proportionate, and bounded. The sizing table still carries its predecessor's caption title and label, and nothing references that label yet. Both superseded files stay on disk unreferenced; restore either by swapping the `\input` in `_4_experiments.tex`. Separately, an earlier generation of RQ3 policy, selectivity, and calibration TeX pairs — not these tables — was deleted; do not restore or reference those.
...
- Call the controller's policy a **deadline-constrained, responsiveness-dominant trade-off**. `timeout > delay > admit` is acceptable when describing the product priority informally, but never translate it into a strict lexicographic optimum: that optimum is the corner solution "delay only as a last resort", which the deployed policy does not implement. Section~4 states the trade-off by name so the four RQs read as its verification.
- Section 3.1 states ordered control priorities and margin dynamics only, and carries no scalar cost objective. A `min Σκ(d_i) + Σ(1-a_{i,j})v_{i,j}` formulation was drafted and rejected for three reasons, all of which still apply: convexity of the delay cost does not imply the split the deployed policy performs, because admission is discrete and its quality cost is a step function; `κ` and `v` appear in neither the implementation nor the evaluation, so the `argmin` reads as ornamental; and `π_i` is a completion-to-completion interval that equals a capture's service demand only while the worker stays busy, which is exactly the condition pacing exists to break. Do not reintroduce anything of that shape.
...
- Sections 3.1 and 3.2 were merged into `3_1_overview.tex` on 2026-08-15. `3_2_objective.tex` is deleted, and the four files after it were renumbered down by one to keep file names aligned with subsection numbers: workload model `3_2_workload_model.tex`, admission `3_3_admission.tex`, pacing `3_4_pacing.tex`, implementation `3_5_implementation.tex`. Labels were not renamed, so `sec:model`, `sec:admission`, `sec:pacing`, and `sec:implementation` still resolve; cite subsections by label, never by hard-coded number. They were split before, with a rule policing which subsection owned which fact, and both opened on the same premise. The merged subsection is ordered by two run-in headings, one per half of its title — `Controller architecture.` for the controller and its two modules, `Control objective.` for the constraint, the recursion, the two levers, the asymmetry, and the cost ranking. It carried three until 2026-08-21, when `Safety and responsiveness.` and `Cross-shot margin dynamics.` were merged: the pair delay/skip was introduced twice across the heading boundary, once as the two product costs and once as the two levers on \(\delta_i\) and \(\pi_i\). Inside the merged block the order is load-bearing — constraint, recursion, levers with the asymmetry, the identity-not-a-control-law caveat, then the cost ranking. The caveat has to sit between the equation and the ranking so the responsiveness-dominant preference reads as a product priority rather than a consequence of Equation~\ref{eq:margin-recursion}, which is the misreading the old heading boundary used to prevent; and the block must end on the named trade-off, which is what `_4_experiments.tex` cites. Keep each fact stated exactly once, and keep the asymmetry next to the levers the equation exposes rather than in the opening. `\label{sec:objective}` belongs on the `\subsection` line: `_4_experiments.tex` references it, and a `\label` placed after a run-in heading silently anchors to the preceding float instead.
