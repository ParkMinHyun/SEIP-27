...
- **Use \(T\) for the live remaining Capture Timeout window in both control modules.** In admission, \(T_{i,j}\) is capture \(i\)'s remaining window at admission point \(j\). In pacing, \(T_i\) is the remaining window read for the newest committed capture at capture \(i\)'s pacing decision, and \(d_i\) applies to the next capture opportunity. Do not reintroduce \(R_{i,j}\) for the admission-side reading.
