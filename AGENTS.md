...
- **Use \(T\) for the live remaining Capture Timeout window in both control modules.** In admission, \(T_{i,j}\) is capture \(i\)'s remaining window at admission point \(j\). In pacing, \(t_i\) is the decision time after capture \(i\), \(T_i\) is the newest committed capture's remaining window at \(t_i\), and \(d_i\) applies to the next capture opportunity. Do not reintroduce \(R_{i,j}\) for the admission-side reading.
