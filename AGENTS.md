...
- Keep the minimum realized deadline margin printed, and characterize its tail instead of trimming it. Reporting only P5 would hide the worst observation in the collection (0.11% of budget against a P5 of 7.20%), which is the first thing an industrial reviewer asks for. The supporting facts are in `data/rq3/estimator/thin_margin_tail.csv`; never upgrade them into a bounded-margin or guaranteed-deadline claim, and leave the baseline counterfactual to RQ1.
- The minimum belongs in the table note, not in a per-class column: the class rows print `Slack P5` alone, and the note states the tightest observation beside the tail count and the backlog context that explains it. A bare min column reads as "no timeout was luck", and per class it is not always the informative statistic (on the 14-decision floor block min and P5 are 4.39% and 4.53%). Do not restore the column, and do not drop the number from the note.
