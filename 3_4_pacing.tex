...
These predictions account for stages that admission has disabled.
...
Pacing computes the callback delay in milliseconds as
\begin{equation}
  d_i=\left\lceil
...
Pacing therefore obtains \(t^{\mathrm{end}}\) by accumulating the point estimates \(\hat P\) of every draft sequence running or waiting in the queue.
...
Pacing then removes the disabled stages from the configured sequence \(\mathcal K\), obtaining the effective sequence \(\mathcal K^{\mathrm{eff}}\).
Because pacing decisions for later captures may occur while it is still queued, pacing adds each draft sequence to \(t^{\mathrm{end}}\) at its capture's decision, using a provisional estimate \(\hat P^{\mathrm{last}}\).
At each draft sequence start, pacing sets \(\hat P^{\mathrm{last}}\) to \(\hat P(\mathcal K^{\mathrm{eff}})\).
...
Pacing therefore reconstructs \(t^{\mathrm{end}}\) at each draft sequence completion, starting from the current time and adding updated estimates for the stages recorded with each queued draft sequence.
